...
import { FindStoreByWhatsappUseCase } from "./find-store-by-whatsapp";

describe("FindStoreByWhatsappUseCase", () => {
	let citiesRepository: InMemoryCitiesRepository;
	let storesRepository: InMemoryStoresRepository;
	let sut: FindStoreByWhatsappUseCase;

	beforeEach(() => {
		citiesRepository = new InMemoryCitiesRepository();
		storesRepository = new InMemoryStoresRepository(citiesRepository);
		sut = new FindStoreByWhatsappUseCase(storesRepository);
	});

	it("should be able to find a store by whatsapp", async () => {
		...
		const { store } = await sut.execute({ whatsapp: "31999999999" });
	...
	it("should throw error when store is not found", async () => {
		await expect(sut.execute({ whatsapp: "99999999999" })).rejects.toThrow(
			"Store not found",
		);
	});
