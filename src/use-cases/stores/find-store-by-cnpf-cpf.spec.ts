...
describe("FindStoreByCnpjCpfUseCase", () => {
	...
	let sut: FindStoreByCnpjCpfUseCase;

	beforeEach(() => {
		citiesRepository = new InMemoryCitiesRepository();
		storesRepository = new InMemoryStoresRepository(citiesRepository);
		sut = new FindStoreByCnpjCpfUseCase(storesRepository);
	});

	it("should be able to find a store by CNPJ/CPF", async () => {
		...
		const { store } = await sut.execute({ cnpjcpf: "12345678901234" });
	...
	it("should find the correct store when multiple stores exist", async () => {
		...
		const { store } = await sut.execute({ cnpjcpf: "11111111111111" });

		expect(store.cnpjcpf).toBe("11111111111111");
