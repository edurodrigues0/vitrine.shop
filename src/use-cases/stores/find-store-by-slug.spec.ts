...
import { FindStoreBySlugUseCase } from "./find-store-by-slug";

describe("FindStoreBySlugUseCase", () => {
	let citiesRepository: InMemoryCitiesRepository;
	let storesRepository: InMemoryStoresRepository;
	let sut: FindStoreBySlugUseCase;

	beforeEach(() => {
		citiesRepository = new InMemoryCitiesRepository();
		storesRepository = new InMemoryStoresRepository(citiesRepository);
		sut = new FindStoreBySlugUseCase(storesRepository);
	});

	it("should be able to find a store by slug", async () => {
		...
		const { store } = await sut.execute({ slug: "lojinha-doce-mel" });

		expect(store).toBeTruthy();
		expect(store.slug).toBe("lojinha-doce-mel");
	...
	it("should find the correct store when multiple stores exist", async () => {
		...
		const { store } = await sut.execute({ slug: "store-1" });
	...
	it("should return store with all properties", async () => {
		...
		const { store } = await sut.execute({ slug: "store-slug" });
