...
import { FindActiveByDriverIdUseCase } from "./find-active-by-driver-id";

describe("FindActiveByDriverIdUseCase", () => {
	let ridesRepository: InMemoryRidesRepository;
	let sut: FindActiveByDriverIdUseCase;
	...
	beforeEach(() => {
		...
		sut = new FindActiveByDriverIdUseCase(ridesRepository);
	});
	...
	it("should throw error when driver has no active ride", async () => {
		await expect(
			sut.execute({
				driverId: "driver-id-1",
			}),
	...
	it("should throw error when driver ride is COMPLETED", async () => {
		const createdRide = await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
	...
	it("should throw error when driver ride is CANCELLED", async () => {
		const createdRide = await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
	...
	it("should throw error when driver ride is REJECTED", async () => {
		const createdRide = await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
	...
	it("should return only active ride for specific driver", async () => {
		...
		const driver2Ride = await ridesRepository.create({
		...
		// Busca corrida do driver-id-2
		const { ride } = await sut.execute({
			driverId: "driver-id-2",
		});

		expect(ride).toBeDefined();
		expect(ride?.id).toBe(driver2Ride.id);
		expect(ride?.driverId).toBe("driver-id-2");
	});
