...
import { FindAllByPassengerIdUseCase } from "./find-all-by-passenger-id";

describe("FindAllByPassengerIdUseCase", () => {
	let ridesRepository: InMemoryRidesRepository;
	let sut: FindAllByPassengerIdUseCase;
	...
	beforeEach(() => {
		...
		sut = new FindAllByPassengerIdUseCase(ridesRepository);
	});

	it("should be able to find all rides by passenger id", async () => {
	...
	it("should return empty array when passenger has no rides", async () => {
		const { rides } = await sut.execute({
			passengerId: "passenger-id-1",
		});
	...
	it("should return only rides from specific passenger", async () => {
		const passenger1Ride = await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
			endLocation: mockEndLocation,
		...
		expect(rides[0]?.id).toBe(passenger1Ride.id);
	...
	it("should handle multiple rides with different drivers", async () => {
		await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
		...
		expect(rides[1]?.driverId).toBe("driver-id-2");
		expect(rides[2]?.driverId).toBe("driver-id-3");
	});
	...
	it("should return rides with different distances and amountCentss", async () => {
		await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
		...
		expect(rides[0]?.amountCents).toBe(15.0);
		expect(rides[1]?.distance).toBe(50.0);
		expect(rides[1]?.amountCents).toBe(150.75);
	});
	...
	it("should handle large number of rides", async () => {
		...
		expect(rides.every((ride) => ride.passengerId === "passenger-id-1")).toBe(
			true,
		);
	});

	it("should handle passenger with single ride", async () => {
		const ride = await ridesRepository.create({
			driverId: "driver-id-1",
			passengerId: "passenger-id-1",
			startLocation: mockStartLocation,
		...
		expect(rides).toHaveLength(1);
		expect(rides[0]?.id).toBe(ride.id);
	});
});
